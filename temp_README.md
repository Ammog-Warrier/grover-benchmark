...
- **Quantum Advantage Threshold**: "At an error rate of ~5%, the quantum success probability drops below 1/N (classical random guessing), marking the limit of this hardware's utility for Grover's search."
- **Depth Explosion**: The exponential growth of circuit depth required to decompose multi-controlled gates (MCX). This gate is a "qubit killer" on NISQ hardware because it decomposes into a massive number of CNOTs, dominating the error budget.
...
- **Error Mitigation**: Implementation of Zero Noise Extrapolation (ZNE) or Measurement Error Mitigation.
